                  ...
                  <div key={inv.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3" data-testid={`invoice-${inv.id}`}>
                    <div className="min-w-0">
                      <p className="flex items-center gap-2 text-[13px] font-bold text-[#1C1C1E]">
                        ...
                        {inv.kind_label ? <span className="rounded-full bg-[#F1F1F4] px-2 py-0.5 text-[10.5px] font-semibold text-[#3C3C43]">{inv.kind_label}</span> : null}
                      </p>
                    ...
                    <div className="flex flex-shrink-0 items-center gap-2">
                      <span className="text-[14px] font-bold tabular-nums text-[#1C1C1E]">{formatCurrency(inv.amount_due ?? inv.amount)}</span>
                      ...
                      <button className="icon-button !h-8 !w-8" title="Unduh PDF" onClick={() => downloadFile(`/invoices/${inv.id}/export?format=pdf`, `${String(inv.number).replace(/\//g, "-")}.pdf`)} data-testid={`invoice-pdf-${inv.id}`}><Download size={14} /></button>
                      <button className="icon-button !h-8 !w-8" title="Unduh Excel" onClick={() => downloadFile(`/invoices/${inv.id}/export?format=excel`, `${String(inv.number).replace(/\//g, "-")}.xlsx`)} data-testid={`invoice-xlsx-${inv.id}`}><FileSpreadsheet size={14} /></button>
                    </div>
                  </div>
                );
              })}
