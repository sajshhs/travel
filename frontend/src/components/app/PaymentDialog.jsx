  ...
  const submit = async () => {
    ...
    try {
      const r = await apiClient.post("/payments", {
        booking_id: booking.id, amount: amt, type, method, idempotency_key: idemKey, send_receipt_wa: sendReceipt,
      });
      const rcp = r.data?.receipt;
      const wa = r.data?.receipt_wa;
      if (rcp && wa?.status === "sent") toast.success(`Pembayaran tercatat · kwitansi ${rcp.number} terkirim via WhatsApp`);
      else if (rcp && wa?.status === "failed") toast.warning(`Pembayaran tercatat · kwitansi ${rcp.number} dibuat, WA gagal: ${wa.error || ""}`);
      else if (rcp) toast.success(`Pembayaran tercatat · kwitansi ${rcp.number} diterbitkan`);
      else toast.success("Pembayaran tercatat");
      onOpenChange(false);
      onSaved && onSaved();
    } catch (e) {
        ...
        <div className="space-y-3">
          ...
          {remaining <= 0 ? (
            ...
            <>
              ...
              <label className="flex items-center justify-between gap-3 rounded-[10px] border border-[#E2E3E7] px-3 py-2">
                <span>
                  <span className="block text-[12.5px] font-semibold text-[#1C1C1E]">Kirim kwitansi via WhatsApp</span>
                  <span className="block text-[11px] text-[#6B6B73]">Kwitansi PDF langsung dikirim ke pelanggan setelah tercatat.</span>
                </span>
                <Switch checked={sendReceipt} onCheckedChange={setSendReceipt} data-testid="pay-send-receipt-wa" />
              </label>
            </>
          )}
        </div>
