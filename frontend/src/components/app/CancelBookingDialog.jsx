...
export default function CancelBookingDialog({ open, onOpenChange, booking, onSaved }) {
  ...
  const [sendRefundWa, setSendRefundWa] = useState(false);
  ...
  const submit = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const r = await apiClient.post(`/bookings/${booking.id}/cancel`, {
        reason: reason.trim(),
        cancellation_fee: feeNum,
        refund_amount: refundNum,
        send_refund_wa: refundNum > 0 && sendRefundWa,
      });
      const note = r.data?.refund_note;
      const wa = r.data?.refund_wa;
      if (note && wa?.status === "sent") toast.success(`Booking ${booking.code} dibatalkan · nota refund ${note.number} terkirim via WhatsApp`);
      else if (note && wa?.status === "failed") toast.warning(`Booking dibatalkan · nota refund ${note.number} dibuat, WA gagal: ${wa.error || ""}`);
      else if (note) toast.success(`Booking ${booking.code} dibatalkan · nota refund ${note.number} diterbitkan`);
      else toast.success(`Booking ${booking.code || booking.id} dibatalkan`);
      onOpenChange(false);
      if (onSaved) onSaved();
    } catch (e) {
  ...
  }, [booking, reason, feeNum, refundNum, sendRefundWa, error, onOpenChange, onSaved]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md" data-testid="cancel-booking-dialog">
        ...
        <div className="space-y-3">
          ...
          {refundNum > 0 ? (
            <label className="flex items-center justify-between gap-3 rounded-lg border border-[#E2E3E7] px-3 py-2">
              <span>
                <span className="block text-[12.5px] font-semibold text-[#1C1C1E]">Kirim Nota Refund via WhatsApp</span>
                <span className="block text-[11px] text-[#6B6B73]">Nota refund PDF (bukti pengembalian dana) langsung dikirim ke pelanggan.</span>
              </span>
              <Switch checked={sendRefundWa} onCheckedChange={setSendRefundWa} data-testid="cb-send-refund-wa" />
            </label>
          ) : null}
