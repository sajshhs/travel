...
class CancelBooking(BaseModel):
    ...
    send_refund_wa: Optional[bool] = False  # kirim Nota Refund PDF via WhatsApp setelah dibatalkan
...
class PaymentCreate(BaseModel):
    ...
    send_receipt_wa: Optional[bool] = False  # kirim kwitansi PDF via WhatsApp segera setelah tercatat
...
class InvoiceCreate(BaseModel):
    ...
    amount: Optional[float] = Field(default=None, ge=0)  # override nominal tagihan
    dp_percent: Optional[float] = Field(default=None, ge=0, le=100)
    tax_enabled: Optional[bool] = None  # None → ikut konfigurasi dokumen
    tax_percent: Optional[float] = Field(default=None, ge=0, le=100)
    items: Optional[List[InvoiceItemIn]] = None  # kosong → dari booking (harga dasar + add-on)
