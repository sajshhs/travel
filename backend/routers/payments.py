...
async def create_payment(body: PaymentCreate, user=Depends(FIN)):
    ...
    # Dokumen: sinkron status invoice + kwitansi otomatis (+ kirim WA bila diminta).
    from services.documents import on_payment_recorded
    try:
        doc_res = await on_payment_recorded(db, guarded, doc, user, send_receipt_wa=bool(body.send_receipt_wa))
    except Exception as exc:  # noqa: BLE001 — dokumen tidak boleh menggagalkan pencatatan uang
        doc_res = {"receipt": None, "error": str(exc)[:160]}
    out = safe_doc(doc)
    out["receipt"] = safe_doc(doc_res.get("receipt"))
    out["receipt_wa"] = doc_res.get("wa")
    return out
