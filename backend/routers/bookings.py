...
async def cancel_booking(booking_id: str, body: Optional[CancelBooking] = None, user=Depends(MANAGER)):
    ...
    # Dokumen: Nota Refund otomatis bila ada refund (+ WA opsional dari dialog pembatalan).
    from services.documents import on_booking_cancelled
    doc_res = await on_booking_cancelled(db, booking_id, user, send_wa=bool(body and body.send_refund_wa))
    out = safe_doc(res)
    out["refund_note"] = safe_doc(doc_res.get("refund_note"))
    out["refund_wa"] = doc_res.get("wa")
    return out
